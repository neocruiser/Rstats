...
## Bootstrap aggregation (Bagging) + 1 classifier = RMSE
	lapsed <- system.time(
      Predd <- foreach(i = 1:ite,
                       .combine=cbind,
                       .packages='caret') %dopar% {
                           bagging.index <- sample(1:dim(training)[1],
                                                   # vector list
                                                   size = floor((nrow(training)/m)))
                           Train.me <- train(y~.,
  ...
  # Test set MSE for regression
	RMSE <- mean((predicted - testing[,1])^2)
  output <- list(Time.Lapsed = lapsed,
                 Predicted.Raw.data = predicted,
                 RMSE=RMSE)
  return(output)
}
