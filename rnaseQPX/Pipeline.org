#+TITLE: Framework (2015): Sequence analysis, gene discovery, and molecular interactions
...
** Postdoc II
*** Goals
A) The first set are 4 QPX transcriptomes generated from different strains of the parasite and the idea here is to comparatively assess the profiles (or even phylogenetic relationships) among the different strains.

**** Project I
1. comparer plusieurs transcriptomes
2. un meme protiste (QPX en l'occurence)
3. derivant de souches geographiques differentes
4. evaluer la "relatedness" entre les differentes souches.

**** Project II
B) The second set of data includes transcriptomic data generated on infectious nodules (QPX + hard clam tissue) from diseased clams and I am interested in screening rare QPX transcripts in those samples to identify parasite transcripts produced in clam tissue during infection and possible crosstalk with the host (clam transcripts responding to parasite factors). The graduate student who did the experiment focused her analysis on most abundant transcripts in the nodules (e.g. clam transcripts) and discarded the rare QPX transcripts.

**** Project III
C) The third project will be use RNAseq data to infer functional networks and used machine learning to confirm association vs multifunctinality vs guilt-by-association (GBA)

1. Is there any preference in exon expression in genes between 2 conditions (count exon-exon junctions and use a GLM model from DEXSeq to analyze significance and edge R. see difference [[http://www.nature.com/nprot/journal/v8/n9/full/nprot.2013.099.html][here]])
2. Use R BitSeq to estimate the number of actual transcribed genes
...
*** DATA
1. BWA seqtk FastQC are found in "~/data/"
2. Original data can be found at "~/data/QPX"
3. Sampling from original data can be found at "~/data/QPX/Align"
4. [[http://www.marinemicroeukaryotes.org/project_organisms][Marine Mirobial Eukayote Transcriptome Project]]
5. [[http://data.imicrobe.us/project/view/104][iMicrobe]] hosted data: description of the project and fasta files + ftp
6. [[http://data.imicrobe.us/project/view/104][NCBI]] metadata entries for the project
7. [[http://jcvi.org/metarep/][JCVI METAREP]] annotation protocol for the project 
*** Bibliography
**** First set
...
6. SAM tools [[http://samtools.sourceforge.net/][sourceforge.net]]
7. Count map reads [[http://www-huber.embl.de/users/anders/HTSeq/doc/overview.html][HTSeq]]
8. MEGA-CC (command line) and MEGA-PRO (GUI) -- [[http://megasoftware.net/MEGA7-CC-Quick-Start-Tutorial.pdf][Intro]]
9. BLASTX =command line=
...
18. Aliners of reads to transriptome (tophat2 GSNAP SpliceMap Subread STAR)
19. Burrows-Wheeler Aligner (BWA) [[http://sourceforge.net/projects/bio-bwa/files/][sourceforge.net]] or on [[https://github.com/lh3/bwa][github]]
20. MAUVE [[http://genome.cshlp.org/content/14/7/1394.short][paper]] for genomic alignment & identification of recombination and horizontal transfer.
21. QIIME for bacterial rna seq data processing [[http://qiime.org/][main site]]
22. [[http://www.scfbm.org/content/10/1/8/abstract][PrimerView]] CPAN module for primer design on multiple sequences
23. R and [[https://github.com/qinwf/awesome-R][awesomeR]] (summary of best used packages)
24. Bpipe [[http://docs.bpipe.org/][documentation]]
...
#+BEGIN_SRC shell
ssh -l bassem gordon.sdsc.xsede.org
Monxsede#14
#+END_SRC
Transfer files
#+BEGIN_SRC shell
scp file1 bassem@gordon.sdsc.xsede.org:~/
find . -name "*.NY.fastq.gz" -exec scp {} bassem@gordon.sdsc.xsede.org:/home/bassem/original \;
#+END_SRC
...
Another lightweight tool for displaying alignments is =Tablet Viewer=. [[http://ics.hutton.ac.uk/tablet/][Link]] to download and manual.P
...
=note= interesting perl and awk commands can be found [[http://bioinformatics.cvr.ac.uk/blog/short-command-lines-for-manipulation-fastq-and-fasta-sequence-files/][here]].
