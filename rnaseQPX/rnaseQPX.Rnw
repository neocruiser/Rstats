...
<<loadcc32>>=
counts.SNP <- read.xls("./data/snp.counts.xlsx", sheet = 1)
glimpse(counts.SNP)
@ 
...
<<barplot11>>=
dat <- read.xls("./data/snp.counts.xlsx", sheet = 4)
...
<<refreads>>=
ref.reads <- read.xls("./data/refreads.xlsx", sheet = 1)
head(ref.reads)
y <- ref.reads[1:12, ]
@ 
...
<<mmetsp0098ref>>=
dat <- read.xls("./data/snp.counts.xlsx", sheet = 2)
ggplot(dat,
       aes(x = factor(sample),
           y = counts,
           fill = factor(reference))) + 
...
<<barplotGATK>>=
dat <- read.xls("./data/snp.counts.xlsx", sheet = 3)
custom.colors <- c(col1 = "#b2182b",
                   col2 = "#ef8a62",
                   col3 = "#fddbc7",
                   col4 = "#e0e0e0",
...
<<snphardfilter>>=
dat <- read.xls("./data/hard.snps.xlsx", sheet = 1)
ggplot(dat,
       aes(x = factor(sample),
           y = snps,
           fill = factor(qd)
...
<<combined.loaded>>=
combined <- read.xls("./data/snp.counts.xlsx", sheet = 1)
glimpse(combined)
@ 
...
<<descriptive1>>=
stats <- read.xls("./data/libraries.xlsx", sheet = 1)
        ...
        theme_bw() + 
            coord_flip() +
                geom_text(aes(x = libraries,
                              ...
                              color = "white",
                              hjust = 1.2)) +
                labs(x = "Number of base pair (bp) in the sequenced QPX libraries",
                     y = "QPX libraries")
@ 
...
<<predicted1>>=
predicted <- read.xls("./data/libraries.xlsx", sheet = 3)
...
<<predicted3>>=
predicted <- read.xls("./data/libraries.xlsx", sheet = 4)
...
<<predicted4>>=
predicted <- read.xls("./data/libraries.xlsx", sheet = 5)
...
<<aggregation1>>=
contigs <- read.xls("./data/annot.stats.xlsx", sheet = 1)
...
<<pfam1>>=
pfam <- read.xls("./data/pfam.xlsx", sheet = 1)
pfam
@ 
...
<<pfam6>>=
pfam2 <- read.xls("./data/pfam.xlsx", sheet = 2)
        ...
        geom_bar(stat = "identity") +
            coord_flip() +
                geom_text(aes(x = position,
                              ...
                              hjust = 2,
                              color = "white")) +
                labs(y = "Number of SNPs (x axis) and % (bar labels)",
                     x = "Position of SNPs relative to contigs aligned to reference")
@ 
...
<<hotspots6>>=
dat <- read.xls("./data/hotspots/snps.all.pfam.xlsx", sheet = 1)
...
dat <- read.xls("./data/hotspots/snps.all.pfam.xlsx", sheet = 2)
