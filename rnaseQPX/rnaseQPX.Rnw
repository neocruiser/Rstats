...
How many SNPs can be found outside of each domain? The outside SNPs can be before or after the aligned contig over the reference. The SNP position outside the domains is dependent on the Reference contig length, which was selected through alignment.
<<hotspots4>>=
