...
The data were transformed to increase the correlation coefficient therefore improving detection of strong correlated patterns. (Example of the strength of data transformation and correlation, visit the following \href{https://blog.majestic.com/case-studies/correlation-data-transformations/}{online page}).\\
...
\label{sec:clonal}
The allele frequency can be indicative if the mutation called, whether it is of germline or somatic origin.
Worth investigating if the call is either 100\% or 50\%.
That is all reads show the variant (100\%), it's very likely a germline call. 
Unless the sample is 100\% tumor cells and all tumor cells have the mutation.
If it is a somatic mutation and the samples are of clinical nature, then a normal cell infiltration is inevitable.
The allele frequency for these mutations will be at 30-40\% (\href{https://www.sciencedirect.com/science/article/pii/S0092867412005272}{Zainal 2012}).
\begin{itemize}
\item Flag known somatic mutations using the COSMIC database
\item Flag known germline mutations using the dbSNP database
\item Filtering by allele frequency
\item Distribution of read mismatches (using bamtools XM:O and XT:U, as in unique)
\item \href{https://www.ncbi.nlm.nih.gov/pmc/articles/PMC3290792/table/T1/}{Koboldt 2012} authors of Varscan2 recommended passed somatic mutations
\item Use the \href{http://www.internationalgenome.org/data}{1000 Genomes project}
\end{itemize}

\subsection{Distribution of variants by estimated allele frequency}
\label{subsec:vaf}
...
\subsection{Allele frequencies and variants using different callers}
\label{subsec:freq}
\subsubsection{Individual 1}
\label{subsec:ind1}

Varscan was used to call for variants against the hg19 GRCh37 human genome. 
The amount of variants was reduced based on read depth per site called. 
Stringency was relaxed and the depth was set between 40 and 400 for all individuals.
<<vaf_ind1_varscan>>=
varscan_af <- read.table("./data/pyclone/reduced_variants/output.4_mined.varscan2.snpeff_annot_genome.clinvar.dbsnp.exac.individual_1.61960-61961.txt", header = T, fill = TRUE)
...
Same as above and with logarithmic scaling.
<<vaf_logged_ind1_varscan>>=
...
<<vaf_300total_ind1_varscan>>=
varscan_af <- read.table("./data/pyclone/stringent/output.4_mined.varscan2.snpeff_annot_genome.clinvar.dbsnp.exac.individual_1.61960-61961.txt", header = T, fill = TRUE)

plot.af(df = varscan_af, pval = 1)
@ 

(Logarithmic) Stringency increased to eliminate above 85\% of variants, keeping a total of 300 sites called.
<<vaf_logged_300total_ind1_varscan>>=
plot.af.logged(df = varscan_af, pval = 1)
@ 



Bcftools was also used to call for variants against the hg19 GRCh37 human genome.
\marginnote{\small\color{blue}$\Lsh$ Varscan and bcftools calls for a total of 2000 variants}[0cm]
<<vaf_ind1_bcftools>>=
bcftools_af<- read.table("./data/pyclone/reduced_variants/output.4_mined.bcftools.snpeff_annot_genome.clinvar.dbsnp.exac.individual_1.61960-61961.txt", header = T, fill = T)
...
Same as above with logarithmic scaling.
<<vaf_logged_ind1_bcftools>>=
...
Stringency increased to eliminate above 85\% of variants, keeping a total of 300 sites called.
<<vaf_300total_ind1_bcftools>>=
bcftools_af <- read.table("./data/pyclone/stringent/output.4_mined.bcftools.snpeff_annot_genome.clinvar.dbsnp.exac.individual_1.61960-61961.txt", header = T, fill = TRUE)
plot.baf(df = bcftools_af)
@ 


(Logarithmic) Stringency increased to eliminate above 85\% of variants, keeping a total of 300 sites called.
<<vaf_logged_300total_ind1_bcftools>>=
plot.baf.logged(df = bcftools_af)
@ 


\subsubsection{Individual 2}
\label{subsec:ind2}
Varscan was used to call for variants against the hg19 GRCh37 human genome.
<<vaf_ind2_varscan>>=
varscan_af<- read.table("./data/pyclone/reduced_variants/output.4_mined.varscan2.snpeff_annot_genome.clinvar.dbsnp.exac.individual_2.61962-61963.txt", header = T, fill = NA)
plot.af(df = varscan_af, pval = 1)

@ 

Logarithmic
<<vaf_logarithmic_ind2_varscan>>=
plot.af.logged(df = varscan_af, pval = 1)

@ 

Allele frequency versus p-value of called variants. For each timepoint.
<<vaf_pval_ind2_t1_varscan>>=
plot.pval.t1(df = varscan_af)

@ 

Allele frequency versus p-value of called variants. For each timepoint.
<<vaf_pval_ind2_t2_varscan>>=
plot.pval.t2(df = varscan_af)

@ 

Stringency increased to eliminate above 85\% of variants, keeping a total of 300 sites called.
<<vaf_300total_ind2_varscan>>=
varscan_af <- read.table("./data/pyclone/stringent/output.4_mined.varscan2.snpeff_annot_genome.clinvar.dbsnp.exac.individual_2.61962-61963.txt", header = T, fill = TRUE)

plot.af(df = varscan_af, pval = 1)
@ 

(Logarithmic) Stringency increased to eliminate above 85\% of variants, keeping a total of 300 sites called.
<<vaf_logged_300total_ind2_varscan>>=
plot.af.logged(df = varscan_af, pval = 1)
@ 

Bcftools was used to call for variants against the hg19 GRCh37 human genome.
<<vaf_ind2_bcftools>>=
bcftools_af<- read.table("./data/pyclone/reduced_variants/output.4_mined.bcftools.snpeff_annot_genome.clinvar.dbsnp.exac.individual_2.61962-61963.txt", header = T, fill = NA)
plot.baf(df = bcftools_af)

@ 

Bcftools was used to call for variants against the hg19 GRCh37 human genome.
<<vaf_log_ind2_bcftools>>=
bcftools_af<- read.table("./data/pyclone/reduced_variants/output.4_mined.bcftools.snpeff_annot_genome.clinvar.dbsnp.exac.individual_2.61962-61963.txt", header = T, fill = NA)
plot.baf.logged(df = bcftools_af)
...
Stringency increased to eliminate above 85\% of variants, keeping a total of 300 sites called.
<<vaf_300total_ind2_bcftools>>=
bcftools_af <- read.table("./data/pyclone/stringent/output.4_mined.bcftools.snpeff_annot_genome.clinvar.dbsnp.exac.individual_2.61962-61963.txt", header = T, fill = TRUE)
plot.baf(df = bcftools_af)
@ 


(Logarithmic) Stringency increased to eliminate above 85\% of variants, keeping a total of 300 sites called.
<<vaf_logged_300total_ind2_bcftools>>=
plot.baf.logged(df = bcftools_af)
@ 


\subsubsection{Individual 3}
\label{subsec:ind3}
Varscan was used to call for variants against the hg19 GRCh37 human genome.
<<vaf_ind3_varscan>>=
varscan_af<- read.table("./data/pyclone/reduced_variants/output.4_mined.varscan2.snpeff_annot_genome.clinvar.dbsnp.exac.individual_3.61964-61965.txt", header = T, fill = NA)
plot.af(df = varscan_af, pval = 1)

@ 

Logarithmic
<<vaf_log_ind3_varscan>>=
plot.af.logged(df = varscan_af, pval = 1)

@ 

Allele frequency versus p-value of called variants. For each timepoint.
<<vaf_pval_ind3_t1_varscan>>=
plot.pval.t1(df = varscan_af)

@ 

Allele frequency versus p-value of called variants. For each timepoint.
<<vaf_pval_ind3_t2_varscan>>=
plot.pval.t2(df = varscan_af)

@ 

Stringency increased to eliminate above 85\% of variants, keeping a total of 300 sites called.
<<vaf_300total_ind3_varscan>>=
varscan_af <- read.table("./data/pyclone/stringent/output.4_mined.varscan2.snpeff_annot_genome.clinvar.dbsnp.exac.individual_3.61964-61965.txt", header = T, fill = TRUE)

plot.af(df = varscan_af, pval = 1)
@ 

(Logarithmic) Stringency increased to eliminate above 85\% of variants, keeping a total of 300 sites called.
<<vaf_logged_300total_ind3_varscan>>=
plot.af.logged(df = varscan_af, pval = 1)
@ 
...
Bcftools was used to call for variants against the hg19 GRCh37 human genome.
<<vaf_ind3_bcftools>>=
bcftools_af<- read.table("./data/pyclone/reduced_variants/output.4_mined.bcftools.snpeff_annot_genome.clinvar.dbsnp.exac.individual_3.61964-61965.txt", header = T, fill = T)
...
Logarithmic
<<vaf_log_ind3_bcftools>>=
plot.baf.logged(df = bcftools_af)

@ 

Stringency increased to eliminate above 85\% of variants, keeping a total of 300 sites called.
<<vaf_300total_ind3_bcftools>>=
bcftools_af <- read.table("./data/pyclone/stringent/output.4_mined.bcftools.snpeff_annot_genome.clinvar.dbsnp.exac.individual_3.61964-61965.txt", header = T, fill = TRUE)
plot.baf(df = bcftools_af)
@ 


(Logarithmic) Stringency increased to eliminate above 85\% of variants, keeping a total of 300 sites called.
<<vaf_logged_300total_ind3_bcftools>>=
plot.baf.logged(df = bcftools_af)
@ 


\subsubsection{Individual 4}
\label{subsec:ind4}
Varscan was used to call for variants against the hg19 GRCh37 human genome.
<<vaf_ind4_varscan>>=
varscan_af <- read.table("./data/pyclone/reduced_variants/output.4_mined.varscan2.snpeff_annot_genome.clinvar.dbsnp.exac.individual_4.95615-95616.txt", header = T, fill = NA)
plot.af(df = varscan_af, pval = 1)

@ 

Logarithmic
<<vaf_log_ind4_varscan>>=
plot.af.logged(df = varscan_af, pval = 1)

@ 

Allele frequency versus p-value of called variants. For each timepoint.
<<vaf_pval_ind4_t1_varscan>>=
plot.pval.t1(df = varscan_af)

@ 

Allele frequency versus p-value of called variants. For each timepoint.
<<vaf_pval_ind4_t2_varscan>>=
plot.pval.t2(df = varscan_af)

@ 

Stringency increased to eliminate above 85\% of variants, keeping a total of 300 sites called.
<<vaf_300total_ind4_varscan>>=
varscan_af <- read.table("./data/pyclone/stringent/output.4_mined.varscan2.snpeff_annot_genome.clinvar.dbsnp.exac.individual_4.95615-95616.txt", header = T, fill = TRUE)

plot.af(df = varscan_af, pval = 1)
@ 

(Logarithmic) Stringency increased to eliminate above 85\% of variants, keeping a total of 300 sites called.
<<vaf_logged_300total_ind4_varscan>>=
plot.af.logged(df = varscan_af, pval = 1)
@ 
...
Bcftools was used to call for variants against the hg19 GRCh37 human genome.
<<vaf_ind4_bcftools>>=
bcftools_af<- read.table("./data/pyclone/reduced_variants/output.4_mined.bcftools.snpeff_annot_genome.clinvar.dbsnp.exac.individual_4.95615-95616.txt", header = T, fill = T)
...
Logarithmic
<<vaf_log_ind4_bcftools>>=
plot.baf.logged(df = bcftools_af)

@ 

Stringency increased to eliminate above 85\% of variants, keeping a total of 300 sites called.
<<vaf_300total_ind4_bcftools>>=
bcftools_af <- read.table("./data/pyclone/stringent/output.4_mined.bcftools.snpeff_annot_genome.clinvar.dbsnp.exac.individual_4.95615-95616.txt", header = T, fill = TRUE)
plot.baf(df = bcftools_af)
@ 


(Logarithmic) Stringency increased to eliminate above 85\% of variants, keeping a total of 300 sites called.
<<vaf_logged_300total_ind4_bcftools>>=
plot.baf.logged(df = bcftools_af)
@ 

\subsubsection{Individual 5}
\label{subsec:ind5}
Varscan was used to call for variants against the hg19 GRCh37 human genome.
<<vaf_ind5_varscan>>=
varscan_af<- read.table("./data/pyclone/reduced_variants/output.4_mined.varscan2.snpeff_annot_genome.clinvar.dbsnp.exac.individual_5.95617-95618.txt", header = T, fill = NA)
plot.af(df = varscan_af, pval = 1)
...
Logarithmic
<<vaf_log_ind5_varscan>>=
plot.af.logged(df = varscan_af, pval = 1)

@ 

Allele frequency versus p-value of called variants. For each timepoint.
<<vaf_pval_ind5_t1_varscan>>=
plot.pval.t1(df = varscan_af)

@ 

Allele frequency versus p-value of called variants. For each timepoint.
<<vaf_pval_ind5_t2_varscan>>=
plot.pval.t2(df = varscan_af)

@ 

Stringency increased to eliminate above 85\% of variants, keeping a total of 300 sites called.
<<vaf_300total_ind5_varscan>>=
varscan_af <- read.table("./data/pyclone/stringent/output.4_mined.varscan2.snpeff_annot_genome.clinvar.dbsnp.exac.individual_5.95617-95618.txt", header = T, fill = TRUE)

plot.af(df = varscan_af, pval = 1)
@ 

(Logarithmic) Stringency increased to eliminate above 85\% of variants, keeping a total of 300 sites called.
<<vaf_logged_300total_ind5_varscan>>=
plot.af.logged(df = varscan_af, pval = 1)
@ 

Bcftools was used to call for variants against the hg19 GRCh37 human genome.
<<vaf_ind5_bcftools>>=
bcftools_af<- read.table("./data/pyclone/reduced_variants/output.4_mined.bcftools.snpeff_annot_genome.clinvar.dbsnp.exac.individual_5.95617-95618.txt", header = T, fill = T)
...
Stringency increased to eliminate above 85\% of variants, keeping a total of 300 sites called.
<<vaf_300total_ind5_bcftools>>=
bcftools_af <- read.table("./data/pyclone/stringent/output.4_mined.bcftools.snpeff_annot_genome.clinvar.dbsnp.exac.individual_5.95617-95618.txt", header = T, fill = TRUE)
plot.baf(df = bcftools_af)
@ 


(Logarithmic) Stringency increased to eliminate above 85\% of variants, keeping a total of 300 sites called.
<<vaf_logged_300total_ind5_bcftools>>=
plot.baf.logged(df = bcftools_af)
@ 
...
\subsection{Location of called variants}
\label{subsec:location}
Variants and their distribution based on their known locations obtained from annotations with different data banks, dbSnp, clinVar, and exac.
...
\subsection{Cellular prevalence and clustering of tumor clones}
\label{subsec:prevalence}
PyClone was loaded with metrics that relay the number of reads specific for each allele (allele depth for reference and alternative alleles), copy numbers (ie., normal, minor, major counts, eg., 2-0-2), and position of each called variant.
The output is then inferred from these metrics and cellular prevalence, clonal clusters, and allele frequency are then reported.
Seed of 124565 was used with disconnected density clustering at 10,000 MCMC iterations with a major copy number prior and binomial distribution analysis.


\subsubsection{Variant allele frequency by individual using Varscan2}
\label{subsec:vafInd}
...
Variant allele frequency (VAF) for the individual anonymously labeled \textbf{CNR1002} for Time 1 (T1) and 2 (T2).
<<vaf_varscan2_ind1_clonal>>=
...
VAF for the individual anonymously labeled \textbf{CNR2008} for Time 1 (T1) and 2 (T2).
<<vaf_varscan2_ind2_clonal>>=
plot_frequencies(df = ind.2)
    
@ 

VAF for the individual anonymously labeled \textbf{CNR3008} for Time 1 (T1) and 2 (T2).
<<vaf_varscan2_ind3_clonal>>=
plot_frequencies(df = ind.3)
    
@ 


VAF for the individual \textbf{LY\_CNSrel\_001} for Time 1 (T1) and 2 (T2).
<<vaf_varscan2_ind4_clonal>>=
plot_frequencies(df = ind.4)
    
@ 

VAF for the individual \textbf{LY\_CNSrel\_003} for Time 1 (T1) and 2 (T2).
<<vaf_varscan2_ind5_clonal>>=
plot_frequencies(df = ind.5)
    
@ 


\subsubsection{Cellular prevalence by individual using Varscan2}
\label{subsec:prevalenceInd}
Cellular prevalence for the individual anonymously labeled \textbf{CNR1002} for Time 1 (T1) and 2 (T2).
\marginnote{\small\color{blue}$\Lsh$Prevalence and VAF were generated by pyclone}[0cm]
<<prevalence_varscan2_ind1_clonal>>=
...
Cellular prevalence for the individual anonymously labeled \textbf{CNR2008} for Time 1 (T1) and 2 (T2).
<<prevalence_varscan2_ind2_clonal>>=
plot_prevalence(df = ind.2)
    
@ 

Cellular prevalence for the individual anonymously labeled \textbf{CNR3008} for Time 1 (T1) and 2 (T2).
<<prevalence_varscan2_ind3_clonal>>=
plot_prevalence(df = ind.3)

@ 


Cellular prevalence for the individual anonymously labeled \textbf{LY\_CNSrel\_001} for Time 1 (T1) and 2 (T2).
<<prevalence_varscan2_ind4_clonal>>=
plot_prevalence(df = ind.4)

@ 

Cellular prevalence for the individual anonymously labeled \textbf{LY\_CNSrel\_003} for Time 1 (T1) and 2 (T2).
<<prevalence_varscan2_ind5_clonal>>=
...
\subsubsection{Variant allele frequency by individual using Bcftools}
Number and allocation of called single point mutations between all three exome sequenced individuals.
Distribution below include the position of the SNV, the chromosome, and the name of the gene.
<<venn_bcftools_clonal>>=
## without functions
ind.1 <- read.table("./data/pyclone/stringent/loci.ind1.563505.bcftools.txt", header = T, fill = T) %>%
        mutate(sample_id = gsub("^.*ind","ind", sample_id))
ind.2 <- read.table("./data/pyclone/stringent/loci.ind2.563506.bcftools.txt", header = T, fill = T) %>%
        mutate(sample_id = gsub("^.*ind","ind", sample_id))     
ind.3 <- read.table("./data/pyclone/stringent/loci.ind3.563507.bcftools.txt", header = T, fill = T) %>%
        mutate(sample_id = gsub("^.*ind","ind", sample_id))    
ind.4 <- read.table("./data/pyclone/stringent/loci.ind4.563508.bcftools.txt", header = T, fill = T) %>%
        mutate(sample_id = gsub("^.*ind","ind", sample_id))     
ind.5 <- read.table("./data/pyclone/stringent/loci.ind5.563509.bcftools.txt", header = T, fill = T) %>%
        mutate(sample_id = gsub("^.*ind","ind", sample_id))
...
VAF for the individual anonymously labeled \textbf{CNR2008} for Time 1 (T1) and 2 (T2).
<<vaf_bcftools_ind2_clonal>>=
plot_frequencies(df = ind.2)
@ 

VAF for the individual anonymously labeled \textbf{CNR3008} for Time 1 (T1) and 2 (T2).
<<vaf_bcftools_ind3_clonal>>=
plot_frequencies(ind.3)
@ 


VAF for the individual anonymously labeled \textbf{LY\_CNSrel\_001} for Time 1 (T1) and 2 (T2).
<<vaf_bcftools_ind4_clonal>>=
plot_frequencies(ind.4)
@ 


VAF for the individual anonymously labeled \textbf{LY\_CNSrel\_003} for Time 1 (T1) and 2 (T2).
<<vaf_bcftools_ind5_clonal>>=
plot_frequencies(ind.5)
@ 



\subsubsection{Cellular prevalence by individual using Bcftools}
Cellular prevalence for the individual anonymously labeled \textbf{CNR1002} for Time 1 (T1) and 2 (T2).
\marginnote{\small\color{blue}$\Lsh$Prevalence and VAF were generated by pyclone}[0cm]
<<prevalence_bcftools_ind1_clonal>>=
plot_prevalence(ind.1)
@ 

Cellular prevalence for the individual anonymously labeled \textbf{CNR2008} for Time 1 (T1) and 2 (T2).
<<prevalence_bcftools_ind2_clonal>>=
plot_prevalence(ind.2)
@ 

Cellular prevalence for the individual anonymously labeled \textbf{CNR3008} for Time 1 (T1) and 2 (T2).
<<prevalence_bcftools_ind3_clonal>>=
plot_prevalence(ind.3)
@ 

Cellular prevalence for the individual anonymously labeled \textbf{LY\_CNSrel\_001} for Time 1 (T1) and 2 (T2).
<<prevalence_bcftools_ind4_clonal>>=
plot_prevalence(ind.4)
@ 

Cellular prevalence for the individual anonymously labeled \textbf{LY\_CNSrel\_003} for Time 1 (T1) and 2 (T2).
<<prevalence_bcftools_ind5_clonal>>=
plot_prevalence(ind.5)
@ 



\subsubsection{Variant allele frequency by individual using GATK}
\marginnote{\small\color{blue}$\Lsh$ OLD!! Deprecated}[0cm]
Number and allocation of called single point mutations between all three exome sequenced individuals.
Distribution below include the position of the SNV, the chromosome, and the name of the gene.
<<venn_gatk_clonal>>=
...
Variant allele frequency (VAF) for the individual anonymously labeled \textbf{CNR1002} for Time 1 (T1) and 2 (T2).
<<vaf_gatk_ind1_clonal>>=
plot_frequencies(ind.1)
@ 

\subsubsection{Cellular prevalence by individual using GATK}
Cellular prevalence for the individual anonymously labeled \textbf{CNR1002} for Time 1 (T1) and 2 (T2).
\marginnote{\small\color{blue}$\Lsh$Prevalence and VAF were generated by pyclone}[0cm]
<<prevalence_gatk_ind1_clonal>>=
model.names <- ind.1 %>%
    select(mutation_id, cluster_id, sample_id, cellular_prevalence) %>%
    spread(sample_id, cellular_prevalence)
