...
<<summary_expression>>=
expression <- read.table("data/summary.full.390387.txt", sep = "\t", header = T) %>%
...
expression %>%
    ...
    scale_y_continuous(breaks = pretty(expression$Transcripts, n = 10)) +
    theme(axis.text.x = element_text(vjust = .5,
                                     angle = 45,
                                     size = 8))
...
Density plot summarizing the distribution of how many times the CNS/NOREL/SYST sample classes were correctly predicted against other classes using the list of genes that supposedly represent patients with each type of prognosis.
...
\begin{table}[h]
  ...
  \begin{tabular}{l p{6.2cm} l p{4.5cm} l}
    ...
    23 & Multilayer perceptron network by stochastic gradient descent & \href{https://cran.r-project.org/web/packages/FCNN4R/index.html}{FCNN4R} & size, l2reg, lambda, learn\_rate, momentum, gamma, minibatchsz, repeats & mlpSGD \\
...
accuracy <- read.table("./data/log.Accuracy.performance1.multianalysis.seed14899815.409567.txt", header = TRUE)
kappa <- read.table("./data/log.Kappa.performance1.multianalysis.seed14899815.409567.txt", header = TRUE)
...
Maximum accuracy (95\% CI) registered for machine learning fitting for all classification groups. Iterations for reproducibility were executed 10 times.
...
df <- read.table("./data/log.performance3.full.hyperTuning.seed14899815.409567.txt", header=T)
df$group <- gsub("[0-9]", "", df$group)

df %>%
    ggplot(aes(x = reorder(model, durationMinutes),
               y = durationMinutes)) +
