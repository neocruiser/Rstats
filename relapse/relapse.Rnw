...
\begin{table}
 ...
 \small\addtolength{\tabcolsep}{-3pt}
  ...
  \vspace{-10pt}
  \caption*{ 
    $^*$Comparing samples with patients grouped by either having a central nervous system relapse (CNS), systemic relapse (SYST), no relapse (NOREL) or controls.
    $^{**}$Comparing samples with patients grouped by either having an activated B-cell (ABC) or germinal center B-cell (GCB) diagnosis.
    $^{***}$Comparing samples with patients grouped by either having an tumor occurrence in lymphnodes (LN) in contrast to extra-nodal (EN) presence.
    The tuning parameters were iterated across a random selection grid for best tuning. Grid configuration is found \href{https://github.com/neocruiser/pipelines/blob/master/r/classification.R#L32}{on Github}.
  }
