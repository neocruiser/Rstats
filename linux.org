...
* INSTALL EMACS 24
Latest version [[http://git.savannah.gnu.org/cgit/emacs.git/tree/][on git]]
#+BEGIN_SRC shell
...
#+END_SRC
[ESS] copy ESS/ to /usr/share/emacs24/site-lisp/
** Markdown mode
...
sudo apt-get install libcurl4-gnutls-dev g++ gfortran libxml2-dev

pkgs = c('RCurl', 'XML','httr','knitr','devtools','rgl')
lapply(pkgs, install.packages)
lapply(pkgs, require, character.only = TRUE)

pkgs = c('doParallel', 'lattice', 'doSNOW', 'ggplot2', 'MASS', 'Matrix', 'caret', 'nnet', 'plyr', 'dplyr', 'formatR', 'evaluate', 'foreach', 'Hmisc', 'rbenchmark', 'RColorBrewer', 'reshape', 'reshape2', 'ROAuth', 'SnowballC', 'wordcloud', 'xlsx','kernlab','randomForest','glmulti','leaps','vegan','scatterplot3d','glmnet', 'tidyr')
...
#+BEGIN_SRC shell
echo "# tetst" >> README.md
git init
git add README.md
git commit -m "first commit"
git remote add origin https://github.com/neocruiser/tetst.git
...
#+END_SRC
* NEEDSREVIEW ADDITIONAL LINUX APPS                              :Table:HOLD:
...
| *zsh*                 | chsh -s /bin/zsh                                        | use zsh + [[http://zsh.sourceforge.net/Guide/][userguide]]                        |
...
mkdir first the folder containing the fasta file. This is usefull when subsetting a big database of just using a reference genome.
#+BEGIN_SRC shell
...
find / -name <what> 2>/dev/null (redirect the error messages)
find / -mtime +60
#+END_SRC

** GREP
Delete all files in a directory except selected ones.
#+BEGIN_SRC shell
ls * | grep -v "tex" | xargs rm -f
#+END_SRC
To delete files and directories alike.
#+BEGIN_SRC shell
ls * | grep -v "tex" | xargs rm -rf
#+END_SRC

** GENERAL COMMANDS
Copy the content of one directory into another. (hint: no need to mkdir dir2)
#+BEGIN_SRC shell
cp -r dir1 dir2
#+END_SRC
