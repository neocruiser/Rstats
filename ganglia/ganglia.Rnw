...
After computing correlations between genes, networks were inferred. Networks are organized into clusters and each contain modules of highly correlated and co-functional genes.
The selected network is based on \textbf{power transformation of 7} to avoid selecting low correlated scores, \textbf{threshold of 0.4} for significance, and \textbf{maximum number of genes per module of 100}.
\smallskip
Detail of the scores:
\begin{itemize}
\item \textbf{Nodes}, differentially expressed transcripts
%\item Edges, connections between nodes
\item \textbf{Modules}, group of genes per cluster (represented by different colors)
\item \textbf{Shared modules}, genes corresponding to one module and appear in two clusters or more
\item \textbf{Hubs25}, genes that have at least 25 edges (degree)
\item \textbf{GO}, Gene Ontology terms and not inferred from electronic annotation
\item \textbf{Reactome}, reactome IDs found
\item \textbf{KEGG}, KEGG IDs found
\item \textbf{NA}, non annotated contigs found in any of sequence database but still found differentially expressed
\item \textbf{Isoforms}, transcripts that have been assembled similarly from the same read bins
%\item IPS raw, annotations found in ProDom, TIGRFAM, SUPERFAMILY, PIRSF, Gene3D, Pfam, SMART, PROSITEPATTERNS, COILS, PANTHER, HAMAP, PRINTS (no e-value or frameshift preselection)
\item \textbf{IPS e10}, annotations at a minimum e-value of $10^{-10}$
\item \textbf{Frame1}, first frameshift annotations of translated transcripts at a minimum e-value of $10^{-10}$
\item \textbf{Frame2}, second frameshift annotations of translated transcripts at a minimum e-value of $10^{-10}$
\item \textbf{Frame3}, third frameshift annotations of translated transcripts at a minimum e-value of $10^{-10}$
\item \textbf{PIRSF}, domain classification (\href{http://pir.georgetown.edu/pirwww/dbinfo/pirsf.shtml}{database}), e-value $10^{-10}$
\item \textbf{TIGRFAM}, subfamilies classification (\href{http://www.jcvi.org/cgi-bin/tigrfams/index.cgi}{database}), e-value $10^{-10}$
\item \textbf{SMART}, Swissprot, Trembl, and Ensembl domains (\href{http://smart.embl-heidelberg.de/}{database}), e-value $10^{-10}$
\item \textbf{PROSITE}, functional domains (\href{http://prosite.expasy.org/}{database}), e-value $10^{-10}$
\item \textbf{PRINTS}, conserved motifs in SwissProts, fingerprinting (\href{http://www.bioinf.manchester.ac.uk/dbbrowser/PRINTS/index.php}{database}), e-value $10^{-10}$
\item \textbf{PFAM}, protein domains (\href{http://pfam.xfam.org/}{database}), e-value $10^{-10}$
\item \textbf{COILS}, coiled coil confirmation (\href{http://www.ch.embnet.org/software/COILS_form.html}{database}), e-value $10^{-10}$
\item \textbf{SUPERFAMILY}, domains, phylogeny and taxon (\href{http://supfam.cs.bris.ac.uk/SUPERFAMILY/}{database}), e-value $10^{-10}$
\item \textbf{Gene3D}, domain families (\href{http://gene3d.biochem.ucl.ac.uk/Gene3D/}{database}), e-value $10^{-10}$
\item \textbf{PANTHER}, domains and pathways (\href{http://www.pantherdb.org/}{database}), e-value $10^{-10}$
%\item Max prot length, the maximum length of annotated proteins, e-value $10^{-10}$ (no frameshift preselection)
\item \textbf{Min prot length}, the minimum length of annotated proteins, e-value $10^{-10}$ (no frameshift preselection)
\end{itemize}

<<cluster.stats, out.width='0.95\\linewidth',out.height='.8\\linewidth'>>=
...
A module is a group of highly correlated genes. Each gene belong to one module. A module can appear in more than one cluster.
Below is the composition of each cluster, each separated \textbf{based only on differentially expressed genes} between gills and ganglia oyster tissues.
Hierachical clustering was done on each module separately to group genes within modules using 5000 bootstraps.
\marginnote{\small\color{blue}$\Lsh$ Modules labeled in green contain less than 10 genes, thus hierarchical clustering cannot be done. They are although expressed in Ganglia except module 5.}[0cm]
<<module.stats, out.width='0.95\\linewidth',out.height='.8\\linewidth'>>=
...
Here we show a distribution of the number of edges per node as to showcase hubs and their degrees.
<<degree.hubs>>=
...
Each module consists of assembled transcripts and grouped together based on their gene expression. These genes have been annotated and assigned a pathway (if a hit score was found). At an e-value of $10^{-10}$ the top NR and HMM annotations were retained. One transcript can have one or more annotations within each database. The NR database of NCBI contains many uncharacterized genes or hypothetical protein functions.
\begin{itemize}
\item \textbf{GO}, Gene Ontology terms and not inferred from electronic annotation
\item \textbf{Pathways}, involve either KEGG and/or reactome IDs
\item \textbf{NonA}, non annotated transcripts in either NR or protein domain databases
\item \textbf{Uncharacterized}, the NR database assign this label to genes that have not been assigned a function
\item \textbf{Hypothetical}, the NR database assign this label to half-structured and potentially functional genes
\item \textbf{NR}, Non redundant database from NCBI that contains gene names and species associations, annotations are based on the BLAST algorithm
\item \textbf{HMM}, Hidden Markov Models that annotate peptides based on HMM aligned profiles against 12 protein databases (See Subsection \ref{subsec:cluster})
\end{itemize}

<<pies.per.module, out.width='0.95\\linewidth',out.height='.95\\linewidth'>>=
...
\subsection{GO-terms for all networks}
\label{subsec:gos}
From the 1233 genes connected in the network, 521 GO entries were found.
<<go.terms, out.width='0.95\\linewidth',out.height='.95\\linewidth'>>=
