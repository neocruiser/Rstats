...
Get differentially expressed genes. Compare shared transcripts and TPM between samples. If the script below is ran on a server an R module must be loaded first and =DESeq2= =limma= and =edgeR= installed.
#+BEGIN_SRC shell
#!/bin/bash
#PBS -l nodes=2:ppn=16,walltime=20:00:00
#PBS -N deg.br.eX.44062
#PBS -q long
#PBS -j oe
#PBS -m abe
#PBS sleiman.bassim@stonybrook.edu
#PBS -V
...
dir=$scratch/ganglia/deg.${file}_$id
...
for method in eXpress
do
    for m in DESeq2 edgeR voom
    do
	for i in 3
	do
	    for pval in {1..6}
		...
		for cfold in {1..2}
		do
	    project=$scratch/ganglia/abundance_${file}_${method}_$id
	    ...
	    cd $project
	    $analyze --matrix $project/trans_counts.counts.matrix --method $m --samples_file $matrix --output $dir/$m.$file.$method.$jobid.p$pval.c$cfold.$pbs --contrasts $contrast

	    cd $dir/$m.$file.$method.$jobid.p$pval.c$cfold.$pbs
