...
Check md5sums with available md5 list for each sample before renaming.
#+BEGIN_SRC shell
md5sum -c md5sus.txt
#+END_SRC
...
Merge =fastq.gz= files into =right.fastq.gz= and =left.fastq.gz= for R1 and R2 respectively. A combination assembly with trinity is superior to separate assembly. Create a reference transcriptome for contig alignment. =n.b.= never chose an output file name that contain an =R1= or =R2= tag. This will create a loop and the merge wont ever end. Or just place the output file in a different folder.
#+BEGIN_SRC shell
time find . -name "*R1*fastq.gz" | xargs zcat | gzip -c > test.fq.gz
#+END_SRC

Count the number of reads after merging, both R1 and R2, to insure the merge was successful.
#+BEGIN_SRC shell
time zcat file.R1.fq.gz | grep -i "@acb052" | wc -l >& count_R1_output.log
#+END_SRC


We assemble a transcriptome to later annotate it.
Using =trinity= I assemble the transcriptome with sequencing reads in =MMETSP0098=. Trimmomatic is integrated in trinity, so I use same default parameters used for the reference genome and transcriptome of Steve Roberts. The script below runs on =Comet= from Xsede at SDSC (San Diego Supercomputer). On Comet, jobs are submitted with =sbatch= and followed with =squeue=.
#+BEGIN_SRC shell
#!/bin/bash
...
## when running inchworm only
## add CPU
#--CPU 64

## when running Butterfly
## remove CPU
#+END_SRC

When running trinity on large datasets it is best to cut jobs into phases. Phase I (add =--no_run_chrysalis=) will construct K-mers. Phase II will run Chrysalis and Butterfly for K-mer/read clustering, building the DeGruijn graph, mapping. On oyster data, RNA are sequenced with Illumina TrueSeq stranded RNA kit. This will extract the second strand. So the sense-forward reads will figure in R2 and reverse reads are in R1. The script below runs on =Gordon= from Xsede using virtual memory because of the excess sequence reads. Over 400 million reads can break inchworm phase with trinity. Allocating around 900 GB of RAM in virtual memory solve the problem.
#+BEGIN_SRC shell
#!/bin/bash
#PBS -q vsmp
#PBS -l nodes=1:ppn=256:vsmp
#PBS -l walltime=20:00:00
#PBS -N trinity_all_inchw
...
Count the number of contigs that have a length of 200 or more. Too many contigs are reported with =Butterfly= if more than 1 million reads are used. This is correct, [[http://trinityrnaseq.github.io/trinity_faq.html#ques_why_so_many_transcripts][see]] what are the options.
#+BEGIN_SRC shell
cat Trinity.fasta  | grep "^>TR" | awk '{print $2}' | sed 's/len=//g' | awk '{ if ($1 < 400) print $1 }' | wc -l
#+END_SRC
...
30. QIIME for DNA sequencing data of microbiome analysis [[http://qiime.org/][website]]
31. [[http://www.genomebiology.com/2014/15/3/R46/abstract][Kraken]] for fast read classification for metganomics =taxonomy=
32. List of sequence analysis [[https://en.wikipedia.org/wiki/List_of_sequence_alignment_software][packages]]
33. [[https://www.gatb.fr/leon-read-compression/][Leon]] read compression of fastq/fasta files (up to 22 folds)
...
*** Transfer big files with globus
1. create globus account [[https://www.globus.org/][sign up]]
2. add ssh public key to globus [[http://dev.globus.org/cli/][tutorial]]
3. sign in
   + =ssh <name>@cli.globusonline.org=
4. Globus use cases [[https://www.xsede.org/software/globus#cli][here]]
5. Globus help [[https://support.globus.org/forums/22861518?utm_source=xsede&utm_medium=website&utm_campaign=xsede-user-guide][CLI]]
6. Activate endpoints on Globus before scp transfer [[https://www.globus.org/xfer/ActivateEndpoints][here]] =sdsc#oasis= and =xsede#comet=
   + =ssh $USER@cli.globusonline.org endpoint-activate sdsc#oasis xsede#comet=
7. Transfer
   + =scp -r sdsc#oasis:/oasis/scratch/$USER/temp_project/... xsede#comet:/oasis/scratch-comet/$USER/temp_project/=
8. Globus [[https://www.xsede.org/web/guest/data-transfers#table12][FTP endpoints]] for xsede (old)

*** Deleting large files
The traditional =rm -r= to delete millions of files inside a directory will fail.
First option is to use =rsync=
#+BEGIN_SRC shell
# create empty directory
## target directory contains million files
mkdir empty
rsync -a --delete empty target
#+END_SRC

Super fast option is with perl.
#+BEGIN_SRC shell
cd target
