...
time transeq assembled.contigs.fa peptides.fa -frame=6
#+END_SRC
...
#+BEGIN_SRC shelle
time hmmscan --domtblout C.txt --cpu 4 ../db/Pfam-A.hmm ./peptides/C.peptides.QPXv15.fa
#+END_SRC
...
Extract the contigs with 4 gaps and at least 450 nucleotides aligned to reference for any of the blocks.
#+BEGIN_SRC shell
cat $trinityOutput.fa | sed 's/.len.*$//g' | perl -ne 'if(/^>(\S+)/){$c=$i{$1}}$c?print:chomp;$i{$_}=1 if @ARGV' <(cat $file.psl | awk 'NR>5' | awk '{print $10,$11,$15,$18,$19}'| awk '{if($4<=1) print$0}' | awk '{if($2>=250)print $0}' | sed 's/,/\t/g' |  awk '{if($5>=450||$6>=450||$7>=450||$8>=450||$9>=450) print $1}' | sort - | uniq) - > $contigs.fa
#+END_SRC
