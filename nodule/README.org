...
2. First goal is to identify (QPX) parasite transcripts produced in clam tissue during infection
3. Published work on this data includes: analysis on most abundant transcripts in the nodules (e.g. clam transcripts) while discarding QPX transcripts.
4. Pipeline was constructed to include 2 filtering phases which helps discard non-QPX reads and contigs
5. Preliminary results include discovery of over 1400 proteins for QPX.
6. The clam genome or transcriptome are used in read assembly to identify and discard dominant clam genes.
7. Annotation of discovered contigs is done on 21 different public databases that includes nucleotide and protein sequences.
8. Second goal is to identify gene-for-gene interplay between the host and pathogen
9. Cluster analysis by gene expression patterns between conditions can highlight gene-for-gene interplay. However the low number of available samples will increase the statistical bias.
10. Alternative approach is to use the available information on protein families to trace connection between subfamilies, processes, and functions.
