...
<<chartrim>>=
trim <- read.xls("./data/Classeur1.xlsx", header = T, sheet = "Feuil1")
trim <- trim[1:3, ]
        ...
        geom_text(aes(x = sample,
            ...
            coord_flip() +
                ...
                labs(y = "Number of mapped reads",
                     x = "Sequenced replicated samples")

@ 

\subsection{QPX transcripts assembled without the hosts transcripts}
\label{subsec:mapped}
...
All contigs were then mapped to the reference genome of QPX. The chart shows the number of contigs that maps with an increasing length going from 100 to 500.
...
All contigs were than translated to peptides in 3 frames. Each peptide was than aligned to the whole PFAM library (v28, date: Jul 14 2015).
<<counts5>>=
...
\label{subsec:blast}
BLAST is used at this step to align the contigs found to non redundant (NR), nucleotide (NT), and swissprot databases. Contigs were translated into peptide sequencing with EMBOSS \textit{transeq} in all 3 possible frames.
...
read.xls("./data/interpro.xlsx", sheet = 2) %>%
...
\label{sec:clamGenes}
For each sequenced tissue we assembled a list of transcripts. The A-list theoretically contains QPX genes, B- and C- contain mostly clam genes. Confidence in the A-list is only valid if clam genes were discarded, and QPX only genes remained.

The same strategy of contig assembly is carried out for clam. A, B, C raw reads are aligned to the clam reference transcriptome. Only the aligned reads are used to assemble 3 lists of hypothetically clam transcripts. There is many transcripts so HMMER is used to select the ones containing significant protein domains. The selected transcripts are aligned to the clam reference and only those that align without any gap (between reference and query) are kept.
<<clam1>>=
...
\marginnote{\small\color{blue}$\Lsh$ One sample contig can align to several reference contigs. The latter are mainly length-isoforms of 1 gene.}[0cm]
\begin{itemize}
\item A contig can align entirely to a reference.
...
\subsection{Differential gene expression of the clam}
\label{subsec:diffclam}
Three conditions were evaluated, 2 samples originated from infected animals but from different location, first a nodule and second a not inflamed tissue. 1 sample originated from a healthy animal.
Also the transcriptome of the clam has been reduced in size by removing low aligned contigs and redundant isoforms. 
Accordingly 40\% and 60\% of the contigs have been removed.

<<diff-clam,out.width='.7\\linewidth',out.height='.7\\linewidth'>>=
...
By removing 60\% of the contigs.
<<diff-clam2,out.width='.7\\linewidth',out.height='.7\\linewidth'>>=
...
Without reducing clam transcriptome by removing unnecessary isoforms and low abundant reads.
<<diff-clam3,out.width='.7\\linewidth',out.height='.7\\linewidth'>>=
...
Distribution of differential expression between R packages.
<<pirateplot,out.width='.7\\linewidth',out.height='.7\\linewidth'>>=
require("yarrr")
...
Distribution of differential expression between transcriptome sizes.
<<pirateplot2,out.width='.7\\linewidth',out.height='.7\\linewidth'>>=
df100 <- read.table("./data/summary.100p.txt", header = F) %>% cbind(size = c("All"))
df60 <- read.table("./data/summary.60p.txt", header = F) %>% cbind(size = c("60p"))
df40 <- read.table("./data/summary.40p.txt", header = F) %>% cbind(size = c("40p"))
...
\subsection{Annotation of selected differentially expressed genes}
\label{subsec:annot}
The differentially expressed genes will be selected from edgeR at an adjusted p-value of 10$^{-3}$ with a fold change of 4 using the transcriptome that is reduced by only 40\% (137,022 contigs). 
The annotated contigs are at an e-value of $10^{-10}$ from \textbf{protein domain alignment}.
<<annot-clam>>=
...
Most abundant species with domain annotations when comparing all samples simultaneously.
This show clam genes.
<<annot-clam-species>>=
...
Most abundant species with domain annotations when comparing samples of infected animals only (A vs B).
This show clam genes.
<<annot-clam-species-ab>>=
...
Most abundant species with domain annotations when comparing infected samples A versus healthy animals C.
This show clam genes.
<<annot-clam-species-ac>>=
...
<<differential,out.width='.5\\linewidth',out.height='.45\\linewidth'>>=
...
Annotations at an evalue of $10^{-10}$ of QPX transcripts assembled only from sample As. There is over 600 proteins found in PANTHER, but only 403 that have an alignment length of 20 aa between query and target.
<<ips>>=
...
Distribution of species from PANTHER annotation at evalue of $10^{-10}$ and alignment 20 aa minimum.
<<species>>=
...
What are the enriched functional categories of the 312 differntially expressed genes between the QPX infected clam tissues and the healthy controls.
<<pies>>=
palette <- brewer.pal(12, name = "Paired")
df <- read.table("./data/degs.pie.txt", header = T, sep = "")
