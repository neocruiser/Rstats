...
\section{Quality controls after trimming bad reads}
\label{subsec:trim}
...
Different trimming parameters where tested. 
The plot shows the differences between nature off adapters (colors) and the combination of the trimming parameters (shapes).
<<chartrim>>=
...
All contigs were than aligned to the reference genome QPX. The chart shows the number of contigs (both genomic and mRNA sequenced) that align with an increasing length of 100>200>300>400>500.
...
All contigs were than translated to peptides in 6 frames. Each frame peptide was than aligned to the whole PFAM library (v28, date: Jul 14 2015).
<<counts5>>=
